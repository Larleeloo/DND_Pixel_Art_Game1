 ...
 * - Taking output consumes input items
 * - Removing any input clears the output
 * - Works with both alchemy (forward) and reverse crafting (deconstruction) modes
 *
 * Usage:
 *   AlchemyTableUI ui = new AlchemyTableUI(false);  // false = alchemy, true = reverse
 *   ui.open(screenWidth/2 - 150, screenHeight/2 - 100);
 *   // In update loop:
...
public class AlchemyTableUI {
    ...
    /**
     * Creates an alchemy table UI.
     *
     * @param reverseMode false for normal alchemy, true for reverse crafting
     */
    public AlchemyTableUI(boolean reverseMode) {
        ...
        if (reverseMode) {
            accentColor = new Color(180, 100, 255);  // Purple
            titleColor = new Color(220, 150, 255);
        } else {
            accentColor = new Color(100, 220, 150);  // Green
            titleColor = new Color(150, 255, 180);
        }
    ...
    /**
     * Opens the UI at the specified position.
     */
    public void open(int centerX, int centerY) {
        ...
        isOpen = true;
        System.out.println("AlchemyTableUI: Opened in " + (reverseMode ? "reverse" : "alchemy") + " mode");
    }
    ...
    public boolean addItem(String itemId, int count) {
        ...
        // For reverse mode, only accept items in the first slot
        if (reverseMode) {
            if (inputSlots[0].isEmpty()) {
                inputSlots[0].setItem(itemId, count);
                updateRecipeReverse();
                return true;
            }
            return false;
        ...
        // Normal alchemy mode - find first empty input slot
        for (int i = 0; i < 3; i++) {
    ...
    /**
     * Updates the output based on input item (reverse crafting mode).
     */
    private void updateRecipeReverse() {
        if (inputSlots[0].isEmpty()) {
            outputSlot.clear();
            currentRecipe = null;
            return;
        ...
        String itemId = inputSlots[0].itemId;
        ...
        if (!reverseRecipes.isEmpty()) {
            ...
            // Show first ingredient as output (others will be given when taken)
            outputSlot.setItem(currentRecipe.ingredients.get(0), 1);
            System.out.println("AlchemyTableUI: Reverse recipe found - " + currentRecipe.name);
        } else {
            outputSlot.clear();
            currentRecipe = null;
            System.out.println("AlchemyTableUI: Item cannot be deconstructed: " + itemId);
        }
    }

    /**
     * Takes the output item, consuming inputs.
     */
    private void takeOutput() {
        if (currentRecipe == null || outputSlot.isEmpty()) return;

        if (reverseMode) {
            // Reverse crafting - give all ingredients
            ...
            // Consume input
            inputSlots[0].stackCount--;
            if (inputSlots[0].stackCount <= 0) {
                inputSlots[0].clear();
            }

            // Update for next item in stack
            updateRecipeReverse();

        } else {
            // Normal alchemy - give result
            if (itemProducedCallback != null) {
                itemProducedCallback.onItemProduced(currentRecipe.result, currentRecipe.resultCount);
            }

            // Notify about consumed items
    ...
    private void drawTitle(Graphics2D g2d) {
        ...
        // Title text
        String title = reverseMode ? "DECONSTRUCTION TABLE" : "ALCHEMY TABLE";
        ...
        String subtitle = reverseMode
            ? "Place item to break down"
            : "Combine items to craft";
